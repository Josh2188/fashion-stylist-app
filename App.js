...
import { getFirestore, collection, addDoc, query, where, onSnapshot, doc, setDoc, getDocs, deleteDoc } from 'firebase/firestore';
import { getStorage, ref, uploadString, getDownloadURL, deleteObject } from "firebase/storage";

// Lucide React Icons for modern UI
const createSvgIcon = (path) => ({ color = 'currentColor', size = 24, ...props }) => (
...
// --- Firebase and Gemini API Configuration ---
// Your provided Firebase project configuration
...
const appId = 'fashion-stylist-app'; // Using your project ID as the app ID
...
export default function App() {
    ...
    const [view, setView] = useState('suggestions'); // suggestions, manual, add, gallery
        ...
        const unsubscribe = onAuthStateChanged(auth, (currentUser) => {
            if (currentUser) {
                ...
                signInAnonymously(auth).catch(authError => {
                    ...
                    setError("無法進行匿名驗證，請檢查您的 Firebase 設定。");
                });
            }
        });
        return () => unsubscribe();
    ...
    useEffect(() => {
        ...
        // Note: The path for profiles is now simplified as it's within a single app context.
        const profilesQuery = query(collection(db, `users/${user.uid}/profiles`));
    ...
    useEffect(() => {
        ...
        setLoading(prev => ({ ...prev, gallery: true, suggestions: true }));
        // Note: The path for clothingItems is now simplified.
        const itemsQuery = query(collection(db, `clothingItems`), where("userId", "==", user.uid), where("profileId", "==", activeProfile.id));
        const unsubscribeItems = onSnapshot(itemsQuery, (snapshot) => {
            const itemsData = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
            setClothingItems(itemsData);
            setTops(itemsData.filter(item => item.category === 'top'));
            setBottoms(itemsData.filter(item => item.category === 'bottom'));
            setLoading(prev => ({ ...prev, gallery: false }));
            if (view === 'suggestions' && itemsData.length > 0) {
                 generateSuggestions(itemsData.filter(item => item.category === 'top'), itemsData.filter(item => item.category === 'bottom'));
            } else {
            ...
            setError("讀取衣物資料失敗。");
            setLoading(prev => ({ ...prev, gallery: false, suggestions: false }));
        });
    ...
    const callGeminiAPI = async (prompt, imageData = null) => {
        const apiKey = ""; // IMPORTANT: Leave this empty for security. It will be handled by the environment.
        let url = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-05-20:generateContent?key=${apiKey}`;
    ...
    const handleAddProfile = async () => {
        if (newProfileName.trim() && user) {
            try {
                const docRef = await addDoc(collection(db, `users/${user.uid}/profiles`), { name: newProfileName.trim() });
                const newProfile = { id: docRef.id, name: newProfileName.trim() };
    ...
    const handleImageUpload = async (event) => {
        ...
        reader.onloadend = async () => {
            ...
            const categoryPrompt = "這是一件上半身衣物（top）還是一件下半身衣物（bottom）？請只回答 'top' 或 'bottom'。";
            const category = await callGeminiAPI(categoryPrompt, base64String);
            ...
            if (category === 'top' || category === 'bottom') {
                ...
                try {
                    ...
                    setView('gallery');
                     ...
                     setError("上傳圖片或儲存資料失敗。");
                }
            } else {
                setError("AI 無法識別這件衣物，請換一張更清晰的照片試試。");
            }
    ...
    const generateSuggestions = async (currentTops, currentBottoms) => {
        ...
        while (newSuggestions.length < attempts) {
            let top, bottom;
            let pairKey;
            let maxTries = 10; // Avoid infinite loop
            do {
            ...
            if(maxTries === 0) break; // Could not find a unique pair, exit loop
            ...
            try {
                ...
                const suggestionPrompt = "這是一套服裝搭配，請用繁體中文給出一句簡短（最多20字）且吸引人的穿搭建議。";
                const comment = await callGeminiAPI(suggestionPrompt, [topBase64, bottomBase64]);

                newSuggestions.push({ top, bottom, comment: comment || "試試這個組合！" });
            } catch (error) {
                console.error("Error fetching images for suggestion:", error);
                newSuggestions.push({ top, bottom, comment: "清新的組合，適合今天！" });
            }
        }
    ...
    const getManualSuggestion = async () => {
        ...
        try {
            ...
            const suggestionPrompt = "這是一套使用者自己搭配的服裝，請用繁體中文以專業且鼓勵的語氣給出建議（約30-40字）。";
            const comment = await callGeminiAPI(suggestionPrompt, [topBase64, bottomBase64]);
            setManualSuggestion(comment || "很棒的選擇！這是一個經典的搭配。");
        } catch (error) {
            console.error("Error getting manual suggestion:", error);
            setError("無法獲取 AI 建議，請稍後再試。");
            setManualSuggestion("您的搭配很有創意！");
        }
    ...
    const confirmDeleteItem = async () => {
        ...
        try {
            ...
            if (manualSelection.top?.id === itemToDelete.id) {
                setManualSelection(prev => ({...prev, top: null}));
            }
            if (manualSelection.bottom?.id === itemToDelete.id) {
                setManualSelection(prev => ({...prev, bottom: null}));
            }
        } catch (error) {
    ...
    const renderLoading = (text) => (
        <div className="flex flex-col items-center justify-center p-8 text-gray-500">
            <RefreshCwIcon className="animate-spin h-8 w-8 mb-4" />
        ...
        <div className="bg-gray-100 min-h-screen font-sans">
            <div className="max-w-md mx-auto bg-white shadow-lg min-h-screen relative">
                ...
                <header className="bg-white p-4 border-b sticky top-0 z-10 flex items-center justify-between">
                    <div className="flex items-center">
                        ...
                        <select 
                            ...
                            className="ml-2 font-semibold text-lg border-none bg-transparent focus:ring-0"
                            disabled={profiles.length === 0}
                        >
                            {profiles.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
                        </select>
                    </div>
                    <button onClick={() => setProfileModalOpen(true)} className="p-2 rounded-full hover:bg-gray-100">
                        <PlusIcon />
                    </button>
                </header>
                ...
                <main className="p-4 pb-20">
                    ...
                    {!activeProfile && profiles.length > 0 && renderEmptyState("請選擇一個使用者", "點擊上方名稱來切換或新增使用者。")}
                    {!activeProfile && profiles.length === 0 && renderEmptyState("歡迎使用 AI 穿搭師！", "請點擊右上角的 '+' 來新增第一位使用者。")}
                        ...
                        <>
                            {view === 'suggestions' && (
                                <section>
                                    ...
                                    {loading.suggestions ? renderLoading("AI 正在搭配中...") : (
                                        ...
                                        ) : renderEmptyState("衣櫥空空的...", "請先去「我的衣櫥」新增一些衣物吧！")
                                    )}
                                </section>
                            )}
                                ...
                                <section>
                                    ...
                                    <div>
                                        ...
                                        {tops.length > 0 ? (
                                            <div className="flex overflow-x-auto space-x-3 pb-3 -mx-4 px-4">
                                                {tops.map(item => (
                                                    <img key={item.id} src={item.imageUrl} alt="Top" 
                                                        onClick={() => setManualSelection(prev => ({...prev, top: item}))}
                                                        className={`w-28 h-36 object-cover rounded-lg flex-shrink-0 cursor-pointer border-4 ${manualSelection.top?.id === item.id ? 'border-pink-500' : 'border-transparent'}`}
                                                    />
                                                ))}
                                            </div>
                                        ) : <p className="text-gray-500">還沒有上身衣物喔。</p>}
                                    </div>
                                    <div className="mt-6">
                                        ...
                                        {bottoms.length > 0 ? (
                                            <div className="flex overflow-x-auto space-x-3 pb-3 -mx-4 px-4">
                                                {bottoms.map(item => (
                                                    <img key={item.id} src={item.imageUrl} alt="Bottom" 
                                                        onClick={() => setManualSelection(prev => ({...prev, bottom: item}))}
                                                        className={`w-28 h-36 object-cover rounded-lg flex-shrink-0 cursor-pointer border-4 ${manualSelection.bottom?.id === item.id ? 'border-pink-500' : 'border-transparent'}`}
                                                    />
                                                ))}
                                            </div>
                                        ) : <p className="text-gray-500">還沒有下身衣物喔。</p>}
                                    </div>
                                    ...
                                    {manualSelection.top && manualSelection.bottom && (
                                        <div className="mt-6 text-center">
                                            ...
                                            {manualSuggestion && (
                                                <div className="mt-4 p-4 bg-purple-50 border border-purple-200 rounded-lg text-purple-800">
                                                    <p>{manualSuggestion}</p>
                                                </div>
                                            )}
                                        </div>
                                    )}
                            ...
                            {view === 'add' && (
                                <section className="text-center p-8 border-2 border-dashed rounded-lg">
                                     <h2 className="text-2xl font-bold text-gray-800 mb-4">新增衣物</h2>
                                     <CameraIcon className="mx-auto h-16 w-16 text-gray-400 mb-4"/>
                                     <p className="text-gray-500 mb-6">上傳照片，AI 會自動幫您分類！</p>
                                     <button onClick={() => fileInputRef.current.click()} disabled={loading.upload} className="bg-gray-800 text-white font-bold py-3 px-6 rounded-full w-full disabled:bg-gray-500">
                                        {loading.upload ? 'AI 辨識中...' : '選擇照片'}
                                     </button>
                                     <input type="file" accept="image/*" ref={fileInputRef} onChange={handleImageUpload} className="hidden" />
                                     {loading.upload && <div className="mt-4">{renderLoading("請稍候...")}</div>}
                                </section>
                            )}
                            ...
                            {view === 'gallery' && (
                                <section>
                                    ...
                                    {loading.gallery ? renderLoading("正在整理衣櫥...") : (
                                        ...
                                        ) : renderEmptyState("衣櫥是空的", "點擊下方的「新增衣物」按鈕，開始建立您的數位衣櫥吧！")
                                    )}
                                </section>
                            )}
                        </>
                ...
                <footer className="bg-white border-t fixed bottom-0 left-0 right-0 max-w-md mx-auto z-10 p-2">
                    <nav className="flex justify-around">
                         <button onClick={() => setView('suggestions')} className={`flex flex-col items-center w-full p-2 rounded-lg ${view === 'suggestions' ? 'text-pink-500 bg-pink-50' : 'text-gray-500'}`}>
                            <Wand2Icon />
                            <span className="text-xs font-medium">AI 推薦</span>
                        </button>
                        <button onClick={() => setView('manual')} className={`flex flex-col items-center w-full p-2 rounded-lg ${view === 'manual' ? 'text-pink-500 bg-pink-50' : 'text-gray-500'}`}>
                            <LightbulbIcon />
                            <span className="text-xs font-medium">手動搭配</span>
                        </button>
                         <button onClick={() => setView('add')} className={`flex flex-col items-center w-full p-2 rounded-lg ${view === 'add' ? 'text-pink-500 bg-pink-50' : 'text-gray-500'}`}>
                            <CameraIcon />
                            <span className="text-xs font-medium">新增衣物</span>
                        </button>
                        <button onClick={() => setView('gallery')} className={`flex flex-col items-center w-full p-2 rounded-lg ${view === 'gallery' ? 'text-pink-500 bg-pink-50' : 'text-gray-500'}`}>
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="h-6 w-6"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><line x1="16" y1="13" x2="8" y2="13"></line><line x1="16" y1="17" x2="8" y2="17"></line><polyline points="10 9 9 9 8 9"></polyline></svg>
                            <span className="text-xs font-medium">我的衣櫥</span>
                        </button>
                    </nav>
                </footer>
            </div>
