...
const parseSingleSuggestion = (line) => {
    ...
    try {
        ...
        console.error("解析單行建議時出錯:", e);
        return null;
    }
};

// 【最終修正】加入指數重試機制的輔助函式
const getSingleSuggestionWithRetry = async (model, prompt, maxRetries = 3) => {
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      const result = await model.generateContent(prompt);
      const responseText = result.response.text();
      const suggestion = parseSingleSuggestion(responseText);
      
      if (suggestion) {
        return { status: 'fulfilled', value: suggestion };
      }
      
      console.warn(`第 ${attempt + 1} 次嘗試：AI 回應的格式無效。正在重試...`);
    } catch (error) {
      console.error(`第 ${attempt + 1} 次嘗試失敗，錯誤訊息:`, error.message);
    }

    if (attempt < maxRetries - 1) {
      const delay = Math.pow(2, attempt) * 1000; // 1秒, 2秒
      console.log(`等待 ${delay} 毫秒後進行下一次重試。`);
      await new Promise(resolve => setTimeout(resolve, delay));
    }
  }
  return { status: 'rejected', reason: '所有重試嘗試都未能獲取有效的建議。' };
};
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (task === 'generate_suggestions') {
        ...
        // 【最終修正】使用新的重試函式來發送三次獨立的請求
        const promises = [
            getSingleSuggestionWithRetry(model, fullPrompt),
            getSingleSuggestionWithRetry(model, fullPrompt),
            getSingleSuggestionWithRetry(model, fullPrompt)
        ];
        ...
        results.forEach(result => {
            if (result.status === 'fulfilled') {
                successfulSuggestions.push(result.value);
            } else {
                console.error("一個建議在所有重試後仍然失敗:", result.reason);
            }
        });
        
        return res.status(200).json({ response: successfulSuggestions });
    }
